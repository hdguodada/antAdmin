...
export const copyFilterObjWithWhiteList = (obj: any, whiteList: string[]) => {
  const entries = whiteList.map((i) => [i, obj[i]]);
  return Object.fromEntries(entries);
};

export const transProTableParamsToMyRequest = (params: any) => {
  const a = { ...params, pageNumber: params.current };
  const keywordList = ['keyword', 'pageNumber', 'pageSize'];
  return {
    ...copyFilterObjWithWhiteList(a, keywordList),
    queryFilter: copyFilterObj(a, [...keywordList, 'current']),
  };
};
...
export const calPrice = (values: Partial<PUR.Purchase>, formRef: any, calRpAmount = false) => {
  ...
  if (calRpAmount) {
    ...
    const customerFree = formRef.current?.getFieldValue('customerFree') || 0;
    formRef.current?.setFieldsValue({
      arrears: toDecimal2(amount + customerFree - rpAmount),
      totalArrears: toDecimal2(accountPayableSum + amount + customerFree - rpAmount),
    });
  }
};
